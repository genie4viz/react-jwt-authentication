  ...
  const coinColumns = [
    ...
    },
    {
      ...
      Cell: row => (
        <span
          style={{
            color: row.row.lb_change_30d_tx_size >= 0 ? 'green' : 'red'
          }}
        >
    ...
    },
    {
      ...
      accessor: 'lb_premium',
      Cell: row => (
        <span style={{ color: row.row.lb_premium >= 0 ? 'green' : 'red' }}>
    ...
    },
    {
      ...
      Cell: row => <span>{numberWithCommasDecimals(row.row.lb_volatility_30_days, 2)}</span>,
      sortMethod: (first, second) => first - second,
      width: '8%'
    }
  ];
      ...
      {coins.length > 0 ? (
        <div>
          <div className="row" style={{ width: '100%', display: 'flex', padding: 0, margin: 0 }}>
            ...
            >
              <span style={{ fontSize: '14pt' }}>Localbitcoins transactions by country</span>
            </div>
          </div>
          <ReactTable
